...
| **SYS-20** | **FAILING, not pending.** The candidate ground station ([NIDAR-GSC](https://github.com/tetraethylmethane/NIDAR-GSC)) exposes `insert_command(lat,lon,alt)`, `jump_to_command`, `set_flight_mode`, `arm/disarm` and `servos` — the exact actions rule 8.16 penalises at −50 each. SYS-20 demands incapability *by construction*, verified by source review. See [`../../ground-station/README.md`](../../ground-station/README.md). |
| **SYS-23** | **FAILING, not pending.** The same codebase polls `https://g.co` every 5 s and switches to online ArcGIS map tiles when connectivity exists — a direct 8.4/8.17 breach that a source inspection under 8.6 would find. Two-line fix; must land before the Pre-Flight Inspection. |
| SYS-25/26/27 | Multi-vehicle support does not exist in the candidate codebase (no `sysid`/`target_system` anywhere), and none of rule 8.14's mission displays — survivor locations, delivery status, per-drone task — are present. 100 binary points depend on these. |
