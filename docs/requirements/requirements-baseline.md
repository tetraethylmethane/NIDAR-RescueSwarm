...
| SYS-25/26/27 | **Resolved.** MAVLink ingest (3 SYSIDs), mission-state listener, map layers, status panel and video wall all built **and rendered** — 107 tests. The mission server now **starts**: DroneKit is out of the mission path, so Python >= 3.10 works (NIDAR-GSC `9f904a7`). Outstanding: a real MediaMTX run, SITL in place of synthetic MAVLink, and flight validation. |
